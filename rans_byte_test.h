...
#define BITS_WRITEOUT 16
#define BITS_MASK ((1u << BITS_WRITEOUT) - 1)
...
static inline RansState RansEncRenorm(RansState x, uint32_t** pptr, uint32_t freq, uint32_t scale_bits)
{
    uint32_t x_max = ((RANS_BYTE_L >> scale_bits) << BITS_WRITEOUT) * freq; // this turns into a shift.
    if (x >= x_max) {
        uint32_t* ptr = *pptr;
        do {
            *--ptr = (uint32_t) (x & BITS_MASK);
            x >>= BITS_WRITEOUT;
        } while (x >= x_max);
        *pptr = ptr;
    }
...
static inline void RansEncPut(RansState* r, uint32_t** pptr, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
    // renormalize
...
static inline void RansEncFlush(RansState* r, uint32_t** pptr)
{
    uint32_t x = *r;
    uint32_t* ptr = *pptr;
...
static inline void RansDecInit(RansState* r, uint32_t** pptr)
{
    uint32_t x;
    uint32_t* ptr = *pptr;
...
static inline void RansDecInitAdvance(RansState* r, uint32_t** pptr)
{
    uint64_t x = *r;

    // renormalize
    x = (x << BITS_WRITEOUT) | **pptr;
    (*pptr)++;
...
static inline void RansDecAdvance(RansState* r, uint32_t** pptr, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
...
static inline void RansEncSymbolInit(RansEncSymbol* s, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
    ...
    s->x_max = ((RANS_BYTE_L >> scale_bits) << BITS_WRITEOUT) * freq;
...
static inline void RansEncPutSymbol(RansState* r, uint32_t** pptr, uint32_t** xptr, RansEncSymbol const* sym)
{
...
static inline void RansDecAdvanceSymbol(RansState* r, uint32_t** pptr, RansDecSymbol const* sym, uint32_t scale_bits)
{
...
// Renormalize.
static inline void RansDecRenorm(RansState* r, uint32_t** pptr)
{
    // renormalize
    uint32_t x = *r;
    if (x < RANS_BYTE_L) {
